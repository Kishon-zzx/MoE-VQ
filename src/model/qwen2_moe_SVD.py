...
# 替换原有 Qwen2MoeMLP 的定义
class Qwen2MoeMLP(nn.Module):
    """带量化误差补偿的 Qwen2MoeMLP 模块（替换原始类）"""
    def __init__(self, config, intermediate_size=None):
        ...
        # 初始化补偿参数（A和B矩阵）
        # 注意：补偿参数的形状需与投影层权重匹配
        self.gate_compensation = LowRankCompensation(
            in_features=self.hidden_size,
            out_features=self.intermediate_size,
            device=self.gate_proj.weight.device
        )
        self.up_compensation = LowRankCompensation(
            in_features=self.hidden_size,
            out_features=self.intermediate_size,
            device=self.up_proj.weight.device
        )
        self.down_compensation = LowRankCompensation(
    ...
    def forward(self, x):
        """带补偿的前向传播"""
        # 原始投影 + 补偿
        gate_output = self.gate_proj(x) + self.gate_compensation(x)
        up_output = self.up_proj(x) + self.up_compensation(x)
        # 激活和下投影 + 补偿
        hidden = self.act_fn(gate_output) * up_output
        down_output = self.down_proj(hidden) + self.down_compensation(hidden)
        return down_output


# 新增低秩补偿模块（确保与你的补偿逻辑一致）
class LowRankCompensation(nn.Module):
    """低秩补偿模块，存储A和B矩阵"""
    def __init__(self, in_features, out_features, device):
        super().__init__()
        # 初始化A和B（形状需匹配投影层权重：gate_proj为 (intermediate_size, hidden_size)）
        self.A = nn.Parameter(torch.randn(out_features, min(in_features, out_features), device=device))
        self.B = nn.Parameter(torch.randn(min(in_features, out_features), in_features, device=device))
    
    def forward(self, x):
        """计算补偿：x @ (A@B)^T = x @ B^T @ A^T"""
