...
class Qwen2MoeMLP(nn.Module):
    def __init__(self, config, intermediate_size=None):
        ...
        self.gate_proj = CreateCompressedLayerHelper(self.hidden_size, self.intermediate_size, config)
        self.up_proj = CreateCompressedLayerHelper(self.hidden_size, self.intermediate_size, config)
        self.down_proj = CreateCompressedLayerHelper(self.intermediate_size, self.hidden_size, config)
...
def CreateCompressedLayerHelper(in_features, out_features, config):
    if not hasattr(config, "compress_config"):
        return nn.Linear(in_features, out_features, bias=True)
    compress_config = config.compress_config
    compression_kwargs = compress_config.get("compression_kwargs", {})
    compression_type = compress_config.get("compression_type", None)
    if isinstance(config.torch_dtype, str):
        config.torch_dtype = getattr(torch, config.torch_dtype)
    if compression_type == "LinearVQ":
        return LinearVQ.blank_init(
            in_features,
            out_features,
        ...
        return SparseLinear.blank_init(
            in_features,
            out_features,
            add_bias=compress_config.get("add_bias", False),
            dtype=config.torch_dtype,
        ...
        return nn.Linear(in_features, out_features, bias=compress_config.get("add_bias", False))
...
class Qwen2MoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2MoeConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = CreateCompressedLayerHelper(config.hidden_size, config.num_attention_heads * self.head_dim, config)
        self.k_proj = CreateCompressedLayerHelper(config.hidden_size, config.num_key_value_heads * self.head_dim, config)
        self.v_proj = CreateCompressedLayerHelper(config.hidden_size, config.num_key_value_heads * self.head_dim, config)
        self.o_proj = CreateCompressedLayerHelper(config.num_attention_heads * self.head_dim, config.hidden_size, config)
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.gate = CreateCompressedLayerHelper(config.hidden_size, config.num_experts, config)
        ...
        self.shared_expert_gate = CreateCompressedLayerHelper(config.hidden_size, 1, config)
