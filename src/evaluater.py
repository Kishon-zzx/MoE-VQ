...
@torch.no_grad()
def ppl_eval(model, tokenizer, datasets=['wikitext2', 'ptb', 'c4'], model_seq_len=2048, batch_size=1, device_map="auto", use_bfloat16=False):
    model.eval()
    ppls = {}
    ...
    for dataset in datasets:
        ...
        for batch in tqdm(test_loader):
            # 关键修复：将输入转换为 LongTensor（整数类型）
            # 嵌入层要求输入必须是整数索引，不能是浮点数
            batch = batch.to(torch.long)  # 转换为长整数类型
            
            # 移动到设备（保持整数类型，后续计算再转精度）
            batch_device = next(model.parameters()).device
            batch = batch.to(batch_device)
            
            # 前向传播
            output = model(batch, use_cache=False)
            lm_logits = output.logits.to(dtype)  # 仅对输出的logits转精度
            
            # 计算损失（移一位）
            shift_logits = lm_logits[:, :-1, :].contiguous()
            shift_labels = batch[:, 1:].contiguous().to(torch.long)  # 标签也需是整数类型
            ...
            # 清理内存
            del output, lm_logits, shift_logits, shift_labels, loss
            torch.cuda.empty_cache()
        
        # 计算PPL（保持原逻辑）
        if total_tokens > 0:
            avg_nll = total_nll / total_tokens
            ppl = np.exp(avg_nll) if avg_nll <= 100 else float('inf')
            ppls[dataset] = ppl
            print(f"{dataset} PPL: {ppl:.4f} (NLL: {avg_nll:.4f})")
        else:
    ...
    for i in range(torch.cuda.device_count()):
        print(f"GPU {i} 内存使用: {torch.cuda.memory_allocated(i)/1024/1024:.2f} MiB")
